...
class BehaviorConfig:
    ...
    car_style_label: int = 1        # 0=cautious, 1=normal, 2=chaotic
    ped_intent_label: int = 1       # 0=wait, 1=cross, 2=retreat
    ped_style_label: int = 0        # 0=cautious, 1=normal, 2=chaotic
    moto_intent_label: int = 1
    moto_style_label: int = 1       # 0=cautious, 1=normal, 2=chaotic
...
# Synthetic per-pedestrian-style imperfection (no traci.person.getImperfection
# analogue exists). Spread roughly mirrors CAR_STYLE_PARAMS sigma so the LSTM
# sees comparable distributions across agent types.
PED_SYNTHETIC_SIGMA = {
    "normal_walk":      0.15,
    "running":          0.25,
    "slow_elderly":     0.05,
    "stop_midway":      0.20,
    "hesitant":         0.30,
    "distracted_slow":  0.40,
    "jaywalking_fast":  0.45,
}
...
# Sigma-aligned style relabeling (Phase 0C). Class semantics:
#   0 = cautious (sigma <= 0.10)
#   1 = normal   (0.10 < sigma <= 0.25)
#   2 = chaotic  (sigma > 0.25)
CAR_STYLE_LABELS = {
    "timid":          0,   # sigma 0.05 — cautious
    "nominal":        1,   # sigma 0.15 — normal
    "rule_violating": 1,   # sigma 0.20 — normal
    "aggressive":     2,   # sigma 0.30 — chaotic
    "distracted":     2,   # sigma 0.40 — chaotic
    "erratic":        2,   # sigma 0.50 — chaotic
    "drunk":          2,   # sigma 0.60 — chaotic
}
PED_INTENT_LABELS = {
    "cross_left_right": 1, "cross_right_left": 1,
}
PED_STYLE_LABELS = {
    "slow_elderly":    0,  # synthetic sigma 0.05 — cautious
    "normal_walk":     1,  # synthetic sigma 0.15 — normal
    "stop_midway":     1,  # synthetic sigma 0.20 — normal
    "running":         1,  # synthetic sigma 0.25 — normal
    "hesitant":        2,  # synthetic sigma 0.30 — chaotic
    "distracted_slow": 2,  # synthetic sigma 0.40 — chaotic
    "jaywalking_fast": 2,  # synthetic sigma 0.45 — chaotic
}
MOTO_INTENT_LABELS = {
...
MOTO_STYLE_LABELS = {
    "cautious":        0,  # sigma 0.05 — cautious
    "yield_to_ego":    0,  # sigma 0.05 — cautious
    "nominal":         1,  # sigma 0.15 — normal
    "late_brake":      1,  # sigma 0.20 — normal
    "aggressive_fast": 2,  # sigma 0.30 — chaotic
    "swerving":        2,  # sigma 0.50 — chaotic
}
