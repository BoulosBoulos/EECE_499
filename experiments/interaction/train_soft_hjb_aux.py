"""Train Soft-HJB auxiliary critic on the interaction benchmark."""
...
def main():
    ...
    policy = SoftHJBAuxAgent(
        ...
        lambda_soft=float(
            pde_cfg.get("lambda_soft", pde_cfg.get("lambda_hjb", 0.2))
        ),
        lambda_bc=float(pde_cfg.get("lambda_bc", 0.5)),
        lambda_distill=float(pde_cfg.get("lambda_distill", 0.25)),
        lambda_align=float(
            pde_cfg.get("lambda_align", pde_cfg.get("lambda_kl", 0.05))
        ),
        tau_soft=float(pde_cfg.get("tau_soft", pde_cfg.get("temperature", 0.1))),
    ...
    csv_path = os.path.join(args.out_dir, f"train_interaction_soft_hjb_aux_{args.scenario}.csv")
    header = [
        "step", "mean_return", "success_rate", "collision_rate", "row_violation_rate",
        "actor_loss", "vf_loss", "entropy",
        "soft_residual_mean", "anchor_loss", "distill_loss", "actor_align_kl",
    ]
    ...
    while step < args.total_steps:
        ...
        with open(csv_path, "a", newline="") as f:
            csv.writer(f).writerow([
                ...
                f"{metrics.get('soft_residual_mean', 0):.4f}",
                f"{metrics.get('anchor_loss', 0):.4f}",
                f"{metrics.get('distill_loss', 0):.4f}",
                f"{metrics.get('actor_align_kl', 0):.4f}",
            ])
        print(f"[SoftHJB/interaction] step={step} ret={mean_ret:.1f} "
              f"succ={evr['success']:.2f} coll={evr['collision']:.2f}")

    ckpt = os.path.join(args.out_dir, f"model_interaction_soft_hjb_aux_{args.scenario}.pt")
    policy.save(ckpt)
    env.close()
    print(f"Saved: {ckpt}")
