"""Train CBF-PDE auxiliary critic on SUMO T-intersection."""
...
def main():
    ...
    parser.add_argument("--config", default="configs/pde/cbf_aux.yaml",
                        help="(Legacy) yaml; explicit CLI flags below override yaml values")
    # CBF specific Phase 1B flags (defaults from config_frozen_v1.yaml).
    parser.add_argument("--lambda_residual", type=float,
                        default=float(_METHOD_CFG["lambda_residual"]),
                        help="Weight for CBF residual loss")
    ...
    csv_path = os.path.join(output_dir, f"train_cbf_aux_{args.scenario}_{args.ego_maneuver}.csv")
    header = [
        "step", "episode_return", "episode_len", "actor_loss", "vf_loss",
        "collision_count", "collision_rate", "mean_ttc", "min_ttc", "entropy",
        "cbf_residual_mean", "anchor_loss", "bc_loss", "distill_loss", "distill_gap",
        "train_time_per_iter",
    ]
    ...
    for _rs in _resume_candidates:
        _rp = os.path.join(output_dir,
            f"model_cbf_aux_{args.scenario}_{args.ego_maneuver}_step{_rs}.pt")
        if os.path.exists(_rp):
            ...
            for _s in _resume_candidates:
                _sp = os.path.join(output_dir,
                    f"model_cbf_aux_{args.scenario}_{args.ego_maneuver}_step{_s}.pt")
    ...
    while step < args.total_steps:
        ...
        L_residual_safety = float(metrics.get("cbf_residual_mean", 0.0))
        ...
        while next_ckpt_idx < len(ckpt_steps) and step >= ckpt_steps[next_ckpt_idx]:
            ...
            ck_path = os.path.join(output_dir,
                f"model_cbf_aux_{args.scenario}_{args.ego_maneuver}_step{ck_step}.pt")
            policy.save(ck_path)
        ...
        if step >= next_log_step:
            ...
            with open(csv_path, "a", newline="") as f:
                csv.writer(f).writerow([
                    ...
                    metrics.get("cbf_residual_mean", 0), metrics.get("anchor_loss", 0),
                    metrics.get("bc_loss", 0), metrics.get("distill_loss", 0),
                    metrics.get("distill_gap", 0), train_time,
                ])
            print(f"[cbf_aux] step={step} ret={np.mean(ep_returns[-10:]):.2f} "
                  f"coll={coll_count} ttc={mean_ttc:.2f} cbf_res={metrics.get('cbf_residual_mean', 0):.4f}")
            next_log_step = step + log_interval_steps

    ckpt_path = os.path.join(output_dir, f"model_cbf_aux_{args.scenario}_{args.ego_maneuver}.pt")
    policy.save(ckpt_path)
    print(f"Saved cbf_aux to {ckpt_path}")
    ...
    meta = {
        ...
        "lambda_cbf_effective": args.lambda_aux if args.lambda_aux is not None else float(pde_cfg.get("lambda_cbf", 0.2)),
    ...
    meta_path = os.path.join(output_dir, f"meta_cbf_aux_{args.scenario}_{args.ego_maneuver}.json")
    ...
    final_residual_safety = float(metrics.get("cbf_residual_mean", 0.0)) if "metrics" in locals() else 0.0
