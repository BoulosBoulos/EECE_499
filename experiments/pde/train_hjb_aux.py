...
def main():
    ...
    # Hard-HJB specific Phase 1B flags (defaults from config_frozen_v1.yaml).
    parser.add_argument("--lambda_residual", type=float,
                        default=float(_METHOD_CFG["lambda_residual"]),
                        help="Weight for HJB residual loss")
    ...
    parser.add_argument("--lambda_aux", type=float, default=None,
                        help="(Legacy) alias for --lambda_residual; --lambda_residual wins if both given")
    args = parser.parse_args()
    # Phase 1F: warn (don't block) if config has been modified post-freeze.
    _lock = check_config_lock(strict=False)
    if not _lock["matches"] and _lock.get("warning"):
        print(_lock["warning"])
    # Honour the legacy --lambda_aux override if --lambda_residual was left at its default.
    ...
    policy = HJBAuxAgent(
        ...
        lambda_hjb=float(args.lambda_residual),
    ...
    csv_path = os.path.join(output_dir, f"train_hjb_aux_{args.scenario}_{args.ego_maneuver}.csv")
    header = [
    ...
    for _rs in _resume_candidates:
        _rp = os.path.join(output_dir,
            f"model_hjb_aux_{args.scenario}_{args.ego_maneuver}_step{_rs}.pt")
        if os.path.exists(_rp):
            ...
            for _s in _resume_candidates:
                _sp = os.path.join(output_dir,
                    f"model_hjb_aux_{args.scenario}_{args.ego_maneuver}_step{_s}.pt")
    ...
    while step < args.total_steps:
        ...
        L_residual_opt = float(metrics.get("hjb_residual_mean", 0.0))
    ...
    ckpt_path = os.path.join(output_dir, f"model_hjb_aux_{args.scenario}_{args.ego_maneuver}.pt")
    ...
    if args.total_steps >= 50_000 and len(intermediate_ckpts) > 1:
        ...
        for ck_step, ck_path_i in intermediate_ckpts.items():
            # Phase 3 followup: from_checkpoint reconstructs the agent using the
            # architecture saved with the checkpoint, eliminating the hidden_dim
            # mismatch bug that previously crashed this loop.
    ...
    meta_path = os.path.join(output_dir, f"meta_hjb_aux_{args.scenario}_{args.ego_maneuver}.json")
    ...
    final_residual_opt = float(metrics.get("hjb_residual_mean", 0.0)) if "metrics" in locals() else 0.0
