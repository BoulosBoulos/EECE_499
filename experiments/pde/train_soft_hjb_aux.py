"""Train Soft-HJB auxiliary critic on SUMO T-intersection."""
...
def main():
    ...
    parser.add_argument("--config", default="configs/pde/soft_hjb_aux.yaml",
                        help="(Legacy) yaml; explicit CLI flags below override yaml values")
    # Soft-HJB specific Phase 1B flags (defaults from config_frozen_v1.yaml).
    parser.add_argument("--lambda_residual", type=float,
                        default=float(_METHOD_CFG["lambda_residual"]),
                        help="Weight for Soft-HJB residual loss")
    ...
    parser.add_argument("--lambda_actor_kl", type=float,
                        ...
                        help="Weight for actor KL alignment to soft policy")
    ...
    csv_path = os.path.join(output_dir, f"train_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}.csv")
    header = [
    ...
    # Auto-resume: find highest intermediate checkpoint saved from a previous run.
    # Skips already-completed training fractions so the job only does remaining steps.
    _resume_candidates = sorted(
        [s for s in ckpt_steps if s < args.total_steps], reverse=True
    )
    for _rs in _resume_candidates:
        _rp = os.path.join(output_dir,
            f"model_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}_step{_rs}.pt")
        if os.path.exists(_rp):
            ...
            for _s in _resume_candidates:
                _sp = os.path.join(output_dir,
                    f"model_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}_step{_s}.pt")
    ...
    while step < args.total_steps:
        ...
        with open(metrics_csv_path, "a", newline="") as f:
            csv.writer(f).writerow([
                ...
                L_residual_opt, 0.0, L_distill_iter,
        ...
        while next_ckpt_idx < len(ckpt_steps) and step >= ckpt_steps[next_ckpt_idx]:
            ...
            ck_path = os.path.join(output_dir,
                f"model_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}_step{ck_step}.pt")
            policy.save(ck_path)
    ...
    ckpt_path = os.path.join(output_dir, f"model_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}.pt")
    ...
    meta = {
        ...
        "lambda_soft_effective": args.lambda_aux if args.lambda_aux is not None else float(pde_cfg.get("lambda_soft", 0.2)),
    ...
    meta_path = os.path.join(output_dir, f"meta_soft_hjb_aux_{args.scenario}_{args.ego_maneuver}.json")
