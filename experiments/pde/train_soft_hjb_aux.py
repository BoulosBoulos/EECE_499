...
def main():
    ...
    policy = SoftHJBAuxAgent(
        ...
        lambda_soft=args.lambda_aux if args.lambda_aux is not None else float(pde_cfg.get("lambda_soft", 0.2)),
        lambda_bc=float(pde_cfg.get("lambda_bc", 0.5)),
        lambda_distill=float(pde_cfg.get("lambda_distill", 0.25)),
        lambda_align=float(pde_cfg.get("lambda_align", 0.05)),
        tau_soft=float(pde_cfg.get("tau_soft", 0.1)),
