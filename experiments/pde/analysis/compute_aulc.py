...
CANONICAL_METHODS = [
    ...
    "eikonal_aux",
    "cbf_aux",
    "fusion_aux",  # added in future phase, but include now to future-proof
]


def extract_method_name(stem: str) -> str:
    """Extract canonical method name from a result filename stem.

    Uses longest-prefix matching to avoid incorrect truncation (e.g.,
    'soft_hjb_aux' must not be truncated to 'soft', 'eikonal_aux' must
    not be truncated to 'eikonal').
    """
    matches = [m for m in CANONICAL_METHODS if stem.startswith(m)]
    if not matches:
        return "unknown"
...
def main():
    ...
    for fpath in csv_files:
        ...
        method = extract_method_name(stem) if stem else basename
