...
class DRPPO:
    ...
    def load(self, path: str, strict_arch: bool = True):
        ...
        # Phase 3 followup: full architecture verification (arch dict).
        from models.pde.checkpointing import verify_arch
        verify_arch(data.get("arch"), self._arch_dict(), strict=strict_arch, ckpt_path=path)
        # Legacy obs_dim check (kept for old checkpoints without arch).
        if "config" in data and not data.get("arch"):
    ...
    @classmethod
    def from_checkpoint(cls, path: str, device: str = "cpu", **overrides) -> "DRPPO":
        """Reconstruct a DRPPO agent from a checkpoint, using its saved architecture."""
        from models.pde.checkpointing import peek_checkpoint_arch
        arch = peek_checkpoint_arch(path, device=device)
        if not arch:
            # Older drppo checkpoints saved arch implicitly inside config
            data = torch.load(path, map_location=device, weights_only=False)
            cfg = data.get("config") or {}
            arch = {k: cfg[k] for k in ("obs_dim", "n_actions", "hidden_dim") if k in cfg}
        ctor_kwargs = {k: arch[k] for k in ("obs_dim", "n_actions", "hidden_dim") if k in arch}
        ctor_kwargs.update(overrides)
        if "obs_dim" not in ctor_kwargs:
            raise ValueError(f"Checkpoint at {path} has no obs_dim in arch/config; "
