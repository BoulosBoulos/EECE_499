"""CBF-PDE auxiliary critic training agent.

Combines standard PPO (recurrent actor-critic) with an auxiliary CBF critic
on the reduced PDE state. The auxiliary critic is trained with:
  - L_anchor: MSE to GAE returns
  - L_cbf: CBF residual squared (ReLU(-max_a [h_dot + alpha*h]))
...
class CBFAuxAgent:
    """Recurrent PPO + CBF-PDE auxiliary critic agent."""
    ...
    def __init__(
        ...
        self.aux_critic = CBFAuxCritic(in_dim=XI_DIM, hidden_dim=aux_hidden_dim).to(device)
    ...
    def train_step(
        ...
        if extra is not None and "xi_curr" in extra:
            ...
            cbf_res_mean = float(rho.detach().abs().mean())
    ...
    @classmethod
    def from_checkpoint(cls, path: str, device: str = "cpu", **overrides) -> "CBFAuxAgent":
