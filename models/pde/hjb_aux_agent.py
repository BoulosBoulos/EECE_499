"""HJB auxiliary critic training agent.

Combines standard PPO (recurrent actor-critic) with an auxiliary HJB critic
on the reduced PDE state. The auxiliary critic is trained with:
  - L_anchor: MSE to GAE returns
  - L_hjb: HJB residual squared
...
from models.pde.residuals import hjb_residual, pde_q_values
...
class HJBAuxAgent:
    """Recurrent PPO + HJB auxiliary critic agent."""
    ...
    def __init__(
        ...
        self.aux_critic = HJBAuxCritic(in_dim=XI_DIM, hidden_dim=aux_hidden_dim).to(device)
    ...
    def train_step(
        ...
        if extra is not None and "xi_curr" in extra:
            ...
            U_colloc = self.aux_critic(xi_colloc)
            rho = hjb_residual(self.aux_critic, xi_colloc, self.dynamics,
            ...
            if "success_terminal" in extra:
                ...
                if succ.any():
                    ...
                    if len(succ_xi) > 0:
                        ...
                        L_bc = L_bc + ((self.aux_critic(succ_xi) - 200.0) ** 2).mean()
            ...
            hjb_res_mean = float(rho.detach().abs().mean())
        ...
        return {
            ...
            "hjb_residual_mean": hjb_res_mean,
    ...
    def _arch_dict(self) -> dict:
        """Architecture parameters that determine tensor shapes (Phase 3 followup)."""
        return {
    ...
    @classmethod
    def from_checkpoint(cls, path: str, device: str = "cpu", **overrides) -> "HJBAuxAgent":
        """Reconstruct an agent from a checkpoint, using its saved architecture.
        Any kwargs in `overrides` win over the saved arch (for advanced uses).
        """
        ...
        if "obs_dim" not in ctor_kwargs:
            raise ValueError(f"Checkpoint at {path} has no 'arch.obs_dim'; "
                             "pre-Phase-3 legacy checkpoint — pass obs_dim= explicitly.")
