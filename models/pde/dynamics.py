...
def compute_one_step_toa_advantages(
    T_net,
    xi: torch.Tensor,
    dynamics: BehavioralDynamics,
    detach: bool = True,
) -> torch.Tensor:
    """Phase 3F-A: compute Eikonal one-step advantages A_eik(s, a) for all 5 actions.

    Returns a tensor of shape (B, 5) with

        A_eik(s, a) = T(s) - T(f_a(s))

    where T_net is the time-of-arrival critic and f_a is the dynamics one-step
    transition for action a. Higher A_eik means action a saves more time.
    See ``verification/phase3F_A_math_derivation.md`` §4 for the derivation.

    Args:
        T_net: time-of-arrival critic (R^79 -> R)
        xi: current states, shape (B, 79)
        dynamics: BehavioralDynamics instance
        detach: if True (default), compute under torch.no_grad() so that
            gradients of any downstream loss are NOT propagated into T_net.
            This is mandatory for L_distill: see math derivation §6.3.
            Set detach=False only for diagnostic logging where T_net's
            parameters should remain in the autograd graph (rare).

    Returns:
        A_eik: tensor of shape (B, 5) with one row per state, one column per action.
    """
    if detach:
        with torch.no_grad():
            T_curr = T_net(xi)
            if T_curr.dim() > 1:
                T_curr = T_curr.squeeze(-1)
            advantages = []
            for a in range(5):
                xi_next = dynamics.one_step(xi, a)
                T_next = T_net(xi_next)
                if T_next.dim() > 1:
                    T_next = T_next.squeeze(-1)
                advantages.append(T_curr - T_next)
